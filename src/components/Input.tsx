...
const Input = ({Data, register,inputErrorSms,style}:DataFormate) => {
  ...
  return (
    <div className={`flex flex-col ${style}`}>
        ...
        <input className={`border-1 h-[40px] rounded-[7px] font-semibold input ${inputErrorSms[Data.name] && 'border-red-500' }   ${style}`} type={Data.inputType}
         {...register(Data.name)} placeholder={Data.placeholder} maxLength={Data.max} />
